...
export class ChooseATranslation implements ILanguageQuestion {
    ...
    private createOptionButtons(scene: Play) {
        ...
        const bottom = 0;
        const buttonArray = [
            createButton(scene, 'What time is it?', width, height, corner),
            createButton(scene, 'Where are you going?', width, height, corner),
            createButton(scene, 'What time will we reach?', width, height, corner),
            createButton(scene, 'What time is the game?', width, height, corner),
        ];
    ...
    removeAllObjects(scene: Play) {
       ...
       scene.question = new WriteInEnglish(scene);
    }
}
