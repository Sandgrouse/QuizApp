...
export class WhichOfTheseIs implements ILanguageQuestion {
    ...
    private addInstruction(scene: Play) {
        const testWord = 'the car';
        this.instruction = scene.make.text({
            x: -50,
            y: -190,
            text: 'Which of these is ' + testWord + '?',
            style: {
                font: '32px monospace',
                fill: '#000000',
            }
    ...
    private createOptionButtons(scene: Play) {
        const self = this;
        // const icon = scene.rexUI.add.roundRectangle(0, 0, 100, 100, 10, 0xFFFF);
        // const icon = new RoundRectangle(0, 0, 100, 100, 10, 0xFFFF);
        const buttonArray1 = [
            createButton(scene, 'Car', 130, 150, 15, 'image', 1, 20),
            createButton(scene, 'Motto', 130, 150, 15, 'image', 1, 20),
        ];
        const buttonArray2 = [
            createButton(scene, 'Machine', 130, 150, 15, 'image', 1, 20),
            createButton(scene, 'Duro', 130, 150, 15, 'image', 1, 20),
        ];
        ...
        row1.forEachButtton(function (button: ILabel, index: number) {
            // const gameObject = button.getElement('#' + 'space');
            // console.log(button);
        });
