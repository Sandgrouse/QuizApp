...
export class Boot extends Phaser.State implements Fabrique.IState {
    ...
    public static mobileResizeCallback(manager: Phaser.ScaleManager): void {
        ...
        const usedWidth: number = Constants.GAME_WIDTH * Constants.GAME_SCALE;
        const usedHeight: number = Constants.GAME_HEIGHT * Constants.GAME_SCALE;
    ...
    public init(): void {
        ...
        const sessionTime: number = Date.now();
    ...
    public preload(): void {
        ...
        const scale: string = 'x' + Constants.GAME_SCALE + '/';
        ...
        Atlases.preloadList.forEach((assetName: string) => {
            this.game.load.atlas(
                assetName, 'assets/atlases/' + scale + assetName + '.png', 'assets/atlases/' + scale + assetName + '.json'
            );
        });
    ...
    public create(): void {
        ...
        /* TODO: If you DON'T want a custom preloader,
        uncomment this piece and preload the assets lists in the Boot or in the Menu states */
