    ...
    export module Save {
        ...
        export class Game {
            ...
            private save(): void {
                // Save the data
                const data: string = JSON.stringify(<IRecoveredState>{
                    m: this.musicOn,
                    sf: this.sfxOn
                });
            ...
            private restore(): void {
                // restore the data
                const storedData: any = this.game.storage.getItem('sg');
                const storedHash: any = this.game.storage.getItem('h');
                ...
                Promise.all([storedData, storedHash])
                    .then((values: any) => {
                        ...
                        if (data && data !== '') {
                            try {
                                const save: IRecoveredState = JSON.parse(data);
